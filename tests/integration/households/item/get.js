...
describe('Integration - GET /households/:uuid', function() {
  ...
  it('should return 401 with no auth token', async function() {
    const res = await chai.request(server)
      .get(`/households/${user1HouseholdUuid}`)
  ...
  it('should return 200 with the correct auth token', async function() {
    ...
    assert.strictEqual(res.body.data.attributes.name, sampleData.users.user1.lastName);
    assert.strictEqual(res.body.data.id, user1HouseholdUuid);
    assert.strictEqual(res.body.data.type, 'households');
  });
});
