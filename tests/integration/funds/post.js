...
describe('Integration - POST /funds', function() {
  let controllers;
  let models;
  let server;
  ...
  before('create sinon spies', function() {
    createFundSpy = sinon.spy(controllers.FundCtrl, 'createFund');
  });

  after('restore sinon spies', function() {
    createFundSpy.restore();
  });
  ...
  it('should return 401 with no auth token', async function() {
    ...
    assert.strictEqual(createFundSpy.callCount, 0);
  });
  ...
  it('should return 201 with valid data', async function() {
    ...
    // Validate FundCtrl.createFund call.
    assert.strictEqual(createFundSpy.callCount, 1);
    const createFundParams = createFundSpy.getCall(0).args[0];
    assert.isOk(createFundParams.auditApiCallUuid);
    assert.strictEqual(createFundParams.name, sampleData.funds.fund1.name);
