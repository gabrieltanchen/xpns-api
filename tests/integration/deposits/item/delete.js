...
describe('Integration - DELETE /deposits/:uuid', function() {
  let controllers;
  let models;
  let server;
  const testHelper = new TestHelper();

  let deleteDepositSpy;
  ...
  before('create sinon spies', function() {
    deleteDepositSpy = sinon.spy(controllers.FundCtrl, 'deleteDeposit');
  });

  after('restore sinon spies', function() {
    deleteDepositSpy.restore();
  });
  ...
  it('should return 401 with no auth token', async function() {
    const res = await chai.request(server)
      .delete(`/deposits/${user1DepositUuid}`)
    ...
    assert.strictEqual(deleteDepositSpy.callCount, 0);
  });
  ...
  it('should return 404 with the wrong auth token', async function() {
    ...
    assert.strictEqual(deleteDepositSpy.callCount, 1);
    const deleteDepositParams = deleteDepositSpy.getCall(0).args[0];
    assert.isOk(deleteDepositParams.auditApiCallUuid);
    assert.strictEqual(deleteDepositParams.depositUuid, user1DepositUuid);
  });
  ...
  it('should return 204 with the correct auth token', async function() {
    ...
    // Validate FundCtrl.deleteDeposit call.
    assert.strictEqual(deleteDepositSpy.callCount, 1);
    const deleteDepositParams = deleteDepositSpy.getCall(0).args[0];
    assert.isOk(deleteDepositParams.auditApiCallUuid);
    assert.strictEqual(deleteDepositParams.depositUuid, user1DepositUuid);
    ...
    const apiCall = await models.Audit.ApiCall.findOne({
      ...
      where: {
        uuid: deleteDepositParams.auditApiCallUuid,
      },
    });
    assert.isOk(apiCall);
